...
Why not [try it out yourself](https://uvc.nexuspipe.com)?

<script
  src="https://unpkg.com/@nexusuvc/frontend@^1.0.0/vanilla"
  type="module"
></script>

<nexus-uvc publicKey="2222222222222222222222222222222222222222222222222222222222222222" options='{"slider": true,"colours": {"left": "#ff6302","right": "#ffc266","check": "#ffa820"}}'></nexus-uvc>

<!-- may not render in dev due to react being annoying -->
...
- **NPM Package**: With a single command, `pnpm i @nexusuvc/frontend`, NexusUVC can be added to your project using [PNPM](https://pnpm.io) (other package managers will also work just fine), ensuring compatibility with your build system, be it Rollup, Vite, Parcel, or Webpack.
- **CDN Use**: Not using a bundler? No problem. We provide CDN links for NexusUVC, allowing you to include our CAPTCHA with just a few lines of HTML.

#### Framework Agnostic

NexusUVC speaks your language, and it fits right into your tech stack:

- **[Svelte](https://svelte.dev) and [SvelteKit](https://kit.svelte.dev)**: Import NexusUVC as a Svelte component with ease. The CAPTCHA can be added with minimal code, making it a native part of your Svelte application.
...
- **Vue and Nuxt**: We provide first-party bindings to our Vanilla JS implementation for Vue, allowing an extremely simple integration.
- **Other Frameworks**: React, Angular, or something else? NexusUVC is compatible with as many JavaScript frameworks, via the vanilla package, ensuring that no matter what your preference is, we've got you covered.
...
Did you know: All the code that runs on your domain is open-source? You can check it out in the [NexusPIPE/uvc-frontend](https://github.com/NexusPIPE/uvc-frontend) repository on Github!
...
You can simply rewrite the above request in whichever language your server
uses, and it'll just work. More information on the complete returned type can be found [in our documentation](https://docs.nexuspipe.com/uvc/integrating/#server-side-validation).
...
By choosing NexusUVC, you can be confident that you are enhancing your website's security without endangering user privacy, and your users can rest assured that their data won't be maliciously used by a large corperation. It is not just a CAPTCHA solution; it's a commitment to a safer, more secure, and private online experience.
